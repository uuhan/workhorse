...
- cargo-work: added `put` for uploads and OSC 94 progress indicators for both `put` and `get`
- health: expanded output with server `version`, `commit`, `os/arch/family`, default shell, and `ulimit -n`
- observability: added `WH_DEBUG=1` trace-stage logs (`trace_id` + stage) for easier hang and flow debugging
- cargo-work: improved remote patch sync to include both worktree changes and ahead-of-upstream commits (`upstream..HEAD`)
- horsed: unified SSH failure replies and fixed multiple early-return paths that left channels open and caused client blocking
- horsed/cargo-work: improved remote command error handling and shell spawn error visibility to reduce silent hangs
- horsed: fixed `--dir` behavior and improved server operations flow (health/logs/autostart related)
- docs: updated AGENTS + skills index and clarified frontend/backend update and remote deploy workflows
